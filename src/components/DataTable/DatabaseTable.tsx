...
import { ColumnConfig, User } from '../../types/types';
...
export default function DatabaseTable() {
  ...
  const [dataType, setDataType] = useState('achado');
  ...
  const [textButton, setTextButton] = useState('Tipo de Achado')
    ...
    const setupListeners = async () => {
        ...
        switch (dataType) {
            case 'tema':
                ...
                unsubscribe = escutarTemas((data) => { setArrayTopicoAchado(data); setRows(createRows(data)); setIsLoading(false); });
                break;
            case 'achado':
                setTextButton('Tipo de Achado');
                setColumns(createGridColumns(achadoHeader));
                unsubscribe = await escutarAchados((data) => { setArrayAchado(data); setRows(createRows(data)); setIsLoading(false); });
                break;
            case 'processo':
                setTextButton('Processo');
                setColumns(createGridColumns(processoHeader));
                unsubscribe = escutarProcessos((data) => { setArrayProcesso(data); setRows(createRows(data)); setIsLoading(false); });
                break;
            case 'relacionamentos':
                setTextButton('Coleta');
                setColumns(createGridColumns(coletaHeader));
                unsubscribe = await escutarColeta((data) => { setArrayColeta(data); setRows(createRows(data)); setIsLoading(false); });
                break;
            default:
                setIsLoading(false);
                break;
  ...
  const createGridColumns = (headers: ColumnConfig[]): GridColDef[] => {
    return headers.map(header => ({
      ...
      renderCell: (params) => {
        ...
        if (['situacaoAchado', 'situacao'].includes(header.id)) {
            const isAprovado = params.value === true;
    ...
    <Grid sx={{
      ...
      <Paper sx={{
        ...
        <Grid container alignItems="center" spacing={2} sx={{ mb: 2, mt: 1, pl: 2, pr: 2 }}>
          ...
          <Grid item>
            <ModalAddData dataType={dataType} textButton={textButton} user={user as User} />
          </Grid>
          <Grid item>
            <ModalColor />
          </Grid>
